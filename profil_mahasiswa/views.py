...
from users.views import is_user,is_admin
...
def edit_profil(request,user_id):
    profil = get_object_or_404(ProfilMahasiswa,user_id=user_id)
    if request.method == 'POST':
        nama = request.POST.get('nama', '').strip()
        nim = request.POST.get('nim', '').strip()
        jurusan = request.POST.get('jurusan', '').strip()
        ...
        if nim_exist:
            return render(
                request,
                'profil_mahasiswa/edit_profil_mahasiswa.html',
                {
                    'profil': profil,
                    'error_nim': 'NIM sudah terdaftar'
                }
        ...
        return redirect('detail_mahasiswa',user_id=profil.user.id)
    return render(
            request,
            'profil_mahasiswa/edit_profil_mahasiswa.html',
            {
